...
/**
 * 绑定关联账号：userNameOrEmail 定位目标账号，对方密码用于证明当前用户拥有该账号。
...
/** 解除关联（id 是 linkId——关联关系主键，不是目标用户的 userId）。 */
...
export async function switchToLinkedAccount(id: string) {
  return request<ImpersonationResultDto>(`/api/app/account-link/${id}/switch`, {
    method: 'POST',
  });
}
